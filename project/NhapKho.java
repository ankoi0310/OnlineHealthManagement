...
//	JDateChooser ng;
	...
	public NhapKho(String s, DefaultTableModel model) {
		super(s);
		dataModel = model;
		p1 = new JPanel();
...
//				});
			...
			ListRequest.getInstance().addRequest(re);
			dataModel.addRow(new Object[] { re.getName(), re.getSex(), re.getAge(), re.getCMND(), re.getAddress(),
					re.getReason() });

			JOptionPane.showMessageDialog(rootPane, "Them hang hoa thanh cong!");
			this.dispose(); // tắt cái bảng đi
			new NhapKho("Nhap Kho", dataModel); // tạo lại một cái bảng mới (khong còn hiện dữ liệu cũ đã nhập)
//			new NhapKho2("NhapKho2", hh, dataModel);
//			}	
