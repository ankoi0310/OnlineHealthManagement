...
//		this.user = user;
	...
	public void getPatient(int num) {
		if (num == -1) { // day la tao voi CreateRequestView
			new PatientController(create);
//			PatientForm form = new PatientForm(null);
//			form.createView();
		}
		if (num == -2) { // day la tao voi UpdateCreate
			new PatientController(update);
		} else if (num != -1 && create != null) {// day la update của CreateRequestView
			new PatientController(create, num);
		} else { // đây là update của UpdateCreate
	...
	public void CreateRquest(String phone, String address, String proDesc) {
		...
		AppBaseResult result = requestService.createRequest(request,user);
...
//			currentCreate.redraw(patientService.getPatients().getData());
	...
	public void UpdateRequest(Long id, String phone, String address, String proDesc, User user) {
